...
function MultiCouch(args) {
  ...
  this.stdin = (options.interactive) ? process.stdin : "ignore";
  this.stdout = (options.interactive) ? process.stdout : fs.openSync(options.stdout_file, "a");
  this.stderr = (options.interactive) ? process.stderr : fs.openSync(options.stderr_file, "a");
  this.couch = null;
  ...
  this.start = function() {
    ...
    if (process.platform === "win32") {
      ...
      this.couch = spawn(path.join(path.dirname(options.couchdb_path), "erl.exe"),args,{
      ...
      this.couch = spawn(options.couchdb_path, args, {
        stdio:[this.stdin, this.stdout, this.stderr],
        env: process.env
      });
    }
    this.emit("start", this.couch);
  };
  ...
  this.stop = function() {
    ...
    if (process.platform === "win32") {
      ...
      this.couch = spawn(options.couchdb_path, [
          "-d",
          "-p " + options.pid_file + ""
          ], {
          stdio:[this.stdin, this.stdout, this.stderr],
