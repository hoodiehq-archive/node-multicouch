...
function MultiCouch(args) {
  ...
  this.start = function() {
    // do the start
    var iin = (options.repl) ? self.stdin : "ignore";
    var out = (options.repl) ? self.stdout : fs.openSync(options.stdout_file, "a");
    ...
    if (process.platform === "win32") {
      ...
      args = [];
      if (!options.repl) args.push("-b");
      if (!options.repl) args.push("-o " + options.stdout_file);
      if (!options.repl) args.push("-e " + options.stderr_file);
      args.push(
        "-p " + options.pid_file,
