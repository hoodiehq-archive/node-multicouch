...
var system_couch = function() {
  ...
  try {
    bin = which("couchdb");
    // two ../../ because the `couchdb` in .../bin/couchdb
    // counts as a level of hierarchy
    default_ini = path.resolve(bin, '../../etc/couchdb/default.ini');
  } catch (e) {
    ...
    var result = execSync(bin + ' printpath');
    default_ini = path.resolve(result.output, 'etc/couchdb/default.ini');
  }
