...
function MultiCouch(args) {
  ...
  var prefix = args.prefix || (isWindows() ? process.env["TEMP"] : "/tmp");
  var win_ini, win_bin;
  if (isWindows()) {
...
var system_couch = function() {
  ...
  try {
    ...
    if (isMac()) {
      // Couldn't find one in PATH, maybe there is an
  ...
  if (!bin) {
    throw new Error("We couldn't find your CouchDB installation. Try setting the 'COUCHDB_PATH' env variable.");
  }
