...
//Optional install path based on Windows RegKeys.
//To be set by the callback below (only under Win32).
var win_bin_reg = undefined;


/**
 * Callback to complete the retrieval of
 * the CouchDB installation path under Win32.
 *
 * NOTICE: 'stdout' == null if not Win32.
 *
 * @param {object} stdout Content from 'stdout'.
 */
var cb = function(stdout){
  if(stdout){
    var lines = stdout.trim().split(os.EOL);
    var path = /^Inno Setup: App Path\s+REG_SZ\s+(.+)$/.exec(lines[1].trim())[1];
    win_bin_reg = path + '\\bin\\erl.exe';
    console.log('[FOUND INSTALL PATH] ' + path);
  }
}

//This 'require' kicks off the Install-Path-Retrieval
//by 'promisifying' a raw child_process.exec() call.
//The 2nd part (getting data out of a returned Promise)
//must be completed in this scope. That's why we
//send a callback reference to findInstallPath()
//so it can execute the callback at the second 'then'
//in the Promise chain (in 'find-executable-win.js').
//
//Note: findInstallPath() only runs under Win32.
require("./find-executable-win").findInstallPath(cb);
